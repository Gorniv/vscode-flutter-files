    ...
    if (!fileName) {
      ...
      const optionsMap = new Map<string, OptionType>(
        Array.prototype.concat.apply(
          [],
          [...resoureParamsMap, [[showOptionsCmd, OptionType.ShowOptions]]],
        ),
      );
      const resourceOptionsTags = resources.get(type).options
        ? Array.prototype.concat.apply(
            [],
            // tslint:disable-next-line: ter-arrow-parens
            resources.get(type).options.map((op) => optionsCommands.get(op).commands),
          )
        : [];
      ...
      const optionsTokens = optionsString
...
export const showOptionsDialog = async (
  config: IConfig,
  loc: IPath,
  ...
  const optionMap = new Map<string, OptionType>(
    resourceOptions.map((op) => {
  ...
  const resourceOptionsChoices = [...optionMap.entries()].map((option) => {
    ...
    const resourceConfigPath = optionItem.configPath
      ? optionItem.configPath.replace('{resource}', resource.toLocaleLowerCase())
      : '';
    const optionConfiguredDefaultValue = optionItem.configPath
      ? deepValue(config, resourceConfigPath) || ''
      : '';
    ...
    const displayValue =
      optionDefaultValue && optionDefaultValue !== ''
        ? `${optionName} (default: ${optionDefaultValue})`
        : optionName;
    return {
      label: displayValue,
      description: optionItem.description,
      picked: loc.params.includes(optionType),
    } as vscode.QuickPickItem;
  });

  const selectedOptions = await vscode.window.showQuickPick(resourceOptionsChoices, {
    canPickMany: true,
    placeHolder: `Select ${resource} options to override`,
  });
  const selectedOptionTypes = selectedOptions
    // tslint:disable-next-line: ter-arrow-parens
...
export const configureOptionsValues = async (
  config: IConfig,
  loc: IPath,
  resource: ResourceType,
  optionTypes: OptionType[],
): Promise<Map<OptionType, string>> => {
  ...
  await asyncForEach(optionTypes, async (ot) => {
    ...
    const resourceConfigPath = optionItem.configPath
      ? optionItem.configPath.replace('{resource}', resource.toLocaleLowerCase())
      : '';
    const optionDefaultValue = optionItem.configPath
      ? deepValue(config, resourceConfigPath) || ''
      : '';
    ...
    const sortedItems =
      firstItem.toLowerCase() === optionDefaultValue.toString().toLowerCase()
        ? items.reverse()
        : items;
    ...
    const val = optionItem.type
      ? await vscode.window.showQuickPick(sortedItems, params)
      : await vscode.window.showInputBox(params);
...
const parseOptionValue = (value: string) =>
  ['True', 'False'].includes(value) ? JSON.parse(value.toLocaleLowerCase()) : value;

export const mapConfigValues = (
  config: IConfig,
  resource: ResourceType,
  optionsValuesMap: Map<OptionType, string>,
) => {
